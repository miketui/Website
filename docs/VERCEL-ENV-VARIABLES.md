...
Environment Variables → **Production**. Repository history was audited on
July 12, 2026: no real `.env` secret file was ever committed; only examples
were tracked. Secrets still belong in Vercel, never in Git.
...
| Variable | Notes |
|---|---|
| `STRIPE_SECRET_KEY` | Rotation reported complete; server-only |
...
| `STRIPE_PRICE_ID_DAILY_DIRECTIVES_BUNDLE` | $59 bundle price; code has a non-secret verified fallback |
| `STRIPE_PRICE_ID_DAILY_DIRECTIVES_SET_01` … `SET_12` | $7.99 individual set prices; code has verified non-secret fallbacks |
| `STRIPE_PRICE_ID_WORKBOOK` | $19.99 workbook price |
...
| `TURNSTILE_SECRET_KEY` | Confirm rotation/status in the provider dashboard; server-only |
...
| `GA4_API_SECRET` | Only needed for Measurement Protocol server-side events; not required for the standard tag |
...
| `SENTRY_AUTH_TOKEN` | Server-only deployment/build token |
...
| `CRON_SECRET` | Server-only; guards `/api/cron/launch-day` |
...
- `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID`, `VERCEL_TOKEN` — not used by the
  application. Do not commit them; repository history contains no tracked
  real `.env` file.
